...
// ulong = b4 b3 b2 b1
#define ULONG_BYTE4(u) ((u & 0xFF000000) >> 24) // ñàìûé ñòàðøèé áàéò
#define ULONG_BYTE3(u) ((u & 0xFF0000) >> 16)
#define ULONG_BYTE2(u) ((u & 0xFF00) >> 8)
#define ULONG_BYTE1(u) (u & 0xFF)		// ñàìûé ìëàäøèé áàéò
...
{
    unsigned char  dest[6];	// MAC-àäðåñ ïîëó÷àòåëÿ
    unsigned char  src[6];	// MAC-àäðåñ îòïðàâèòåëÿ
    unsigned short type;	// âåðñèÿ: IPv4 0x0800, IPv6 0x86DD, ARP 0x0806
    unsigned char  data[];	// äàííûå
};

struct ETHERNET_ARP
{
    unsigned short hrd;		// Òèï àïïàðàòóðû (Ethernet), 0x0001.
    unsigned short pro;		// Ïðîòîêîë (IP), 0x0800.
    unsigned char  hln;		// Äëèíà àïïàðàòíîãî àäðåñà (MAC), 6 áàéò.
    unsigned char  pln;		// Äëèíà àäðåñà ïðîòîêîëà IP, 4 áàéòà.
    unsigned short op;		// Âèä îïåðàöèè {Çàïðîñ, Îòâåò} = {1, 2}.
    unsigned char  sha[6];	// Àïïàðàòíûé àäðåñ (MAC) îòïðàâèòåëÿ.
    unsigned char  spa[4];	// IP-àäðåñ îòïðàâèòåëÿ.
    unsigned char  tha[6];	// Àïïàðàòíûé àäðåñ (MAC) ïîëó÷àòåëÿ.
    unsigned char  tpa[4];	// IP-àäðåñ ïîëó÷àòåëÿ.
};

typedef struct _IPHeader
{
    unsigned char  ver_len;		// âåðñèÿ è äëèíà çàãîëîâêà
    unsigned char  tos;			// òèï ñåðâèñà ÐÀÇÁÈÒÜ ÍÀ ÔËÀÃÈ(0 1 2 3 ... 7)
    ...
    unsigned long  dest;		// IP-àäðåñ íàçíà÷åíèÿ
    //------------------------------------------------------------
    unsigned short *params;	    // ïàðàìåòðû (äî 320 áèò)
    unsigned char  *data;	    // äàííûå (äî 65535 - äëèíà çàãîëîâêà)
}IPHeader;
...
// Áóôåð äëÿ ïðè¸ìà äàííûõ
BYTE RS_Buffer[MAX_PACKET_SIZE]; // 64 Kb

WSADATA			RS_WSAData;         // Èíèöèàëèçàöèÿ WinSock.
SOCKET			RS_SSocket;         // Cëóùàþùèé ñîêåò.
char			RS_Hostname[128];   // Èìÿ õîñòà (êîìïüþòåðà).
HOSTENT*		RS_Hostinfo;        // Èíôîðìàöèÿ î õîñòå.
SOCKADDR_IN		RS_SocketAddress;   // Àäðåñ õîñòà
static u_long   RS_Flag = 1;        // Ôëàã PROMISC Âêë/âûêë.
NetStat			RS_NetStat;
...
/*
*   ïðåîáðàçîâàíèå ñåòâîãî àäðåñà õîñòà â ñòðîêó
*/
...
/*
*   ïðåîáðàçîâàíèå ip-çàãîëîâêà â ñòðîêó
*/
